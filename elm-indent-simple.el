...
(defun elm-indent-level-2-previous-lines ()
...
(defun elm-indent-lastly ()
  (interactive)
  (cl-multiple-value-bind (previous-line 2nd-previous-line)
      (elm-indent-level-2-previous-lines)
    (cond
...
(defun elm-indent-simple-region (start end)
  "Apply `elm-indent-line' to every line between START and END."
  (save-excursion
    (goto-char start)
    (while (< (point) end)
      (elm-indent-line)
...
(defun elm-indent-forward (&optional arg)
...
(defun elm-indent-backward (&optional arg)
...
(defvar elm-indent-simple-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-c TAB") 'elm-indent-line)
    (define-key map (kbd "<tab>") 'elm-indent-forward)
    (define-key map (kbd "<backtab>") 'elm-indent-backward)
    map))
